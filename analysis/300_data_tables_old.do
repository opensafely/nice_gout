...
use "$projectdir/output/data/data_table.dta", clear
drop if month_year ==. //drop missing
replace outcome_name = "first_flare_drug"
sort outcome_name outcome_desc month_year
...
foreach categorical_var in flare_drug_ flare_source {
	...
	local varlab : value label `categorical_var'
	levelsof `categorical_var' if !missing(`categorical_var'), local(levels)
	...
	foreach level of local levels {
		local ++n
		local lab : label `varlab' `level'
		di as text "  Level `level' = `lab'"
		local newname = "outcome`n'"
		gen `newname' = (`categorical_var' == `level') if !missing(`categorical_var')
